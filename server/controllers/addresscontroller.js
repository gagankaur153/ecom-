...
const getaddress = async(req,res)=>{
    try{
     const userid = req.user.id
     if(!userid) return res.status(400).json({ status: false, message:"user id not find" });
     // const {fullname,state,city,pincode,address} = req.body || {}
     // if(!fullname || !state || !city|| !pincode || !address) return res.status(400).json({status:"false", message:"all fields are required"})
     
     let exisitnguser = await ProductUser.findById(userid).populate("address")
     // if(exisitnguser){
